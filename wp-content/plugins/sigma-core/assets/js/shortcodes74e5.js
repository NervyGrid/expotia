...
Filter faq
-------------------------------------------------------------------------------*/
let triggerFaqMsg = function(){
  let height = $(".sigma_faq_wrapper .row").innerHeight();
  $("#faq-error-msg").toggle(height < 50);
}

$('#faq-search-form').on('keyup change', function() {
...
let filterCategories = function(exp){
  $('.faq_box-filter').each(function() {
    let isMatch = exp.test( $(this).find('h6').text() );
    $(this).toggle(isMatch);
  });
